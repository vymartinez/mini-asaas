...
trait Repository<DomainClass, RepositoryClass extends Repository> {
    ...
    public static RepositoryClass query(Map search = [:]) {
        if (search instanceof GrailsParameterMap) {
            throw new RuntimeException("Utilização do GrailsParameterMap como filtros da query. Para evitar vulnerabilidades, crie um novo Map fazendo o parse somente dos filtros permitidos na consulta.")
        }
    ...
    public <T> T sumAbsoluteWithAlias(String aliasName, String fieldName) {
        String databaseColumnName = StringUtils.camelCaseToSnakeCase(fieldName)
        String outputAliasName = "sum_${aliasName}_${fieldName.capitalize()}"

        return (T) (
            disableSort()
                .addCriteria {
                    projections {
                        projection new DynamicAliasSqlProjection(
                            "sum(abs({${aliasName}}.${databaseColumnName})) as ${outputAliasName}",
                            [outputAliasName] as String[],
                            [BIG_DECIMAL] as Type[]
                        )
                    }
                }
    ...
    /**
     * @deprecated Use o método `sumAbsolute` sem o parâmetro `responseType`
     */
    @Deprecated
    public <T> T sumAbsolute(String fieldName, Class<T> responseType) {
        String databaseColumnName = StringUtils.camelCaseToSnakeCase(fieldName)
        String aliasName = "sum${fieldName.capitalize()}"
